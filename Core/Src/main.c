...
uint32_t last_time = 0; // Last time the main loop ran
...
int main(void)
{
  ...
  MX_ADC1_Init();
  MX_TIM2_Init();
  Flash_RecoverWritePointer();
  HAL_TIM_Base_Start(&htim2);
  bms_instance1.i2cHandle = &hi2c1;
  bms_instance1.bootPort = BOOT_GPIO_Port;
  bms_instance1.bootPin = BOOT_Pin;
  bms_instance2.i2cHandle = &hi2c2;
  bms_instance2.bootPort = BOOT2_GPIO_Port;
  bms_instance2.bootPin = BOOT2_Pin;
  BMS_Service_Init();

  osKernelInitialize();
  NormalTaskHandle = osThreadNew(StartDefaultTask, NULL, &NormalTask_attributes);
  ThermalTaskHandle = osThreadNew(StartTask02, NULL, &ThermalTask_attributes);
  BalancingTaskHandle = osThreadNew(StartTask03, NULL, &BalancingTask_attributes);
  ChargingTaskHandle = osThreadNew(StartTask04, NULL, &ChargingTask_attributes);
  osKernelStart();
  while (1)
  {
  }
	...
	for(;;)
	  {
	    ...
	    Log_Telemetry(BMS_MSG_LEVEL_DEBUG, &telemetry);
	    osDelay(100);
  }
}
...
void StartTask02(void *argument)
{
  ...
  for(;;)
  {
    ...
    float temp1 = TMP100_ReadTemperature(&hi2c1, TMP100_IC1_ADDR);
    float temp2 = TMP100_ReadTemperature(&hi2c1, TMP100_IC2_ADDR);
    PID_Control(temp1, temp2);
    osDelay(100);
  }
}
...
void StartTask03(void *argument)
{
  for(;;)
  {
    if (telemetry.bms_online)
    {
      bool success = EnableBalanceCell(&bms_instance1, telemetry.current_ic1 / 1000.0f);
      if (!success)
      {
        // If balancing fails on BMS 1, try BMS 2
        EnableBalanceCell(&bms_instance2, telemetry.current_ic2 / 1000.0f);
      }
    }
    osDelay(100); // delay to avoid tight loop
  }
}
